...
def scan_market():
    ...
    # เพิ่มตัวนับเทรนด์เพื่อสรุปภาพรวมตลาด
    bullish_count = 0
    bearish_count = 0
    ...
    for coin in COINS:
        ...
        # 📊 ตรวจสอบเทรนด์ของเหรียญรายตัว (อิงจาก EMA 200)
    ...
    # สรุปเทรนด์ภาพรวมตลาดจากจำนวนเหรียญทั้งหมด
    total_scanned = bullish_count + bearish_count
    if bullish_count > bearish_count:
        market_summary = f"🟢 ขาขึ้นเป็นหลัก (Bullish Dominant) | ขึ้น {bullish_count}/{total_scanned} เหรียญ"
    elif bearish_count > bullish_count:
        market_summary = f"🔴 ขาลงเป็นหลัก (Bearish Dominant) | ลง {bearish_count}/{total_scanned} เหรียญ"
    else:
        market_summary = f"⚪ ตลาดไซด์เวย์เลือกทาง (Sideways/Mixed) | ขึ้น {bullish_count} : ลง {bearish_count}"
            
    return buy_signals, sell_signals, market_summary

if __name__ == "__main__":
    print("Starting Comprehensive Screener with Market Trend Summary...")
    buy_list, sell_list, market_summary = scan_market()
    
    # 1. ส่งแจ้งเตือนถ้าเจอสัญญาณช้อนซื้อ
    if buy_list:
        message_buy = f"🎯 [Coindesk Crypto Screener 4H - สัญญาณช้อนซื้อ]\n"
        message_buy += f"📊 ภาพรวมตลาด: {market_summary}" # <--- เพิ่มสรุปเทรนด์ตลาดตรงนี้
        for opt in buy_list:
            message_buy += f"\n\n🪙 เหรียญ: {opt['coin']}"
            message_buy += f"\n📈 เทรนด์เหรียญ: {opt['trend']}" # <--- เพิ่มเทรนด์ของรายเหรียญ
            message_buy += f"\n🚨 รูปแบบ: {opt['type']}"
    ...
    if sell_list:
        message_sell = f"⚠️ [Coindesk Crypto Screener 4H - เตือนโซน Overbought]\n"
        message_sell += f"📊 ภาพรวมตลาด: {market_summary}\n" # <--- เพิ่มสรุปเทรนด์ตลาดตรงนี้
        message_sell += "คำแนะนำ: ราคาวิ่งแรงเกินไป ควรพิจารณาแบ่งขายทำกำไร"
    ...
    if not buy_list and not sell_list:
        print(f"No buy or sell signals triggered. Market Summary: {market_summary}")
